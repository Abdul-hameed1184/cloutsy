...
const services = [
  {
    ...
    data: [
      { id: '101', name: 'Instagram Likes', rate: 150, min: 10, max: 5000, desc: 'Real likes for posts' },
      { id: '102', name: 'Instagram Followers', rate: 250, min: 50, max: 10000, desc: 'Active followers' },
    ],
  },
  {
    title: 'Twitter Services',
    data: [
      { id: '201', name: 'Twitter Retweets', rate: 100, min: 20, max: 2000, desc: 'Boost your tweets' },
    ],
  },
  {
    title: 'Twitter Services',
...
const Services = () =>  {
  ...
  return (
    <section className={`${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'} py-12 px-4 md:px-10`}>
      <div className="max-w-6xl mx-auto space-y-8">
        <h2 className="text-3xl md:text-4xl font-bold text-center">Our Services</h2>

        {services.map((service, i) => (
          <div key={i} className={`border rounded-xl overflow-hidden ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <button
                ...
                >
                  <div className="overflow-auto">
                    <table className="min-w-full table-auto text-sm border-collapse mt-2">
                      <thead>
                        <tr className={`${darkMode ? 'bg-gray-800' : 'bg-gray-100'} text-left`}>
...
{selectedService && (
  <div className="fixed inset-0 z-50 bg-white/10 backdrop-blur-sm flex items-center justify-center">
    <div className={`${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} p-6 rounded-xl w-full max-w-md shadow-lg`}>
      <h3 className="text-xl font-bold mb-4">Service Details</h3>
      <ul className="space-y-2">
        <li><strong>ID:</strong> {selectedService.id}</li>
        <li><strong>Name:</strong> {selectedService.name}</li>
        <li><strong>Rate:</strong> {selectedService.rate}</li>
        <li><strong>Min:</strong> {selectedService.min}</li>
        <li><strong>Max:</strong> {selectedService.max}</li>
        <li><strong>Description:</strong> {selectedService.desc}</li>
      </ul>
      <button
